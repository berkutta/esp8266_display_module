...
void wirecube_ProcessLine(struct Line2d *ret,struct Line3d vec)
{
  float zvt1;
  int xv1,yv1,zv1;
...
void wirecube_render(void) {
        ...
        wirecube_SetVars(); //sets up the global vars to do the conversion.
        int i;
        for(i=0; i<LinestoRender ; i++)
        {
          ORender[i]=Render[i]; // stores the old line segment so we can delete it later.
          wirecube_ProcessLine(&Render[i],Lines[i]); // converts the 3d line segments to 2d.
        }

        for (i=0; i<LinestoRender; i++ )
        {
          graphic_line(Render[i].p0.x, Render[i].p0.y - 30, Render[i].p1.x, Render[i].p1.y - 30);
        }
}